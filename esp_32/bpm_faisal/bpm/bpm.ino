...
// Math Variables
const byte RATE_SIZE = 2; // CHANGED: From 4 to 2 for faster BPM detection
...
void setup()
{
  ...
  particleSensor.setPulseAmplitudeRed(0x1F); // CHANGED: From 0x0A to 0x1F (stronger signal)
  particleSensor.setPulseAmplitudeGreen(0);
  particleSensor.setPulseAmplitudeIR(0x1F); // ADDED: IR LED power for better detection
...
void startSession()
{
  ...
  lastBeatTime = millis(); // ADDED: Initialize beat timer
...
void processSignal(long irValue, long redValue)
{
  if (checkForBeat(irValue) == true)
  {
    ...
    if (delta > 250 && delta < 2000)
    {
      ...
      if (beatsPerMinute < 255 && beatsPerMinute > 40) // CHANGED: Min BPM from 20 to 40 (filters out noise)
      {
